...
<div class="card card-table">
    <div class="card-header">
        <h2>
          <i class="fas fa-hourglass-half me-2" style="margin-right: 10px;"></i>คำขอที่รอพิจารณา
            (<?= count($pending) ?>)
          </h2>
    </div>
    <?php if (!$pending): ?>
    ...
    <table class="tbl">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                  <a href="?id=<?= (int)$r['request_id'] ?>">พิจารณา →</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
<div class="card card-form">
    <div class="card-header">
        <h2>
          <i class="fas fa-file-alt me-2" style="margin-right: 10px;"></i>รายละเอียดคำขอ
    ...
    <div class="card-body" style="padding: 24px;">
        <dl class="kv">
            ...
            <dt>คณะ/สาขา</dt>
            <dd><?= h($selected['faculty'].' / '.$selected['major']) ?></dd>
            <dt>GPA</dt>
            <dd><?= h($selected['gpa']) ?></dd>
            <dt>บริษัท</dt>
            <dd><?= h($selected['company_name']) ?> (<?= h($selected['province']) ?>)</dd>
            <dt>ผู้ติดต่อ</dt>
            <dd><?= h($selected['contact_person']) ?> <?= h($selected['contact_phone']) ?></dd>
            <dt>ตำแหน่ง</dt>
            <dd><?= h($selected['position_title']) ?></dd>
            <dt>ช่วงฝึกงาน</dt>
            <dd><?= h($selected['start_date']) ?> ถึง <?= h($selected['end_date']) ?></dd>
            <dt>รายละเอียด</dt>
            <dd><?= nl2br(h($selected['description'] ?: '-')) ?></dd>
        </dl>
