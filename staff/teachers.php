...
<div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
    <h1>
        <i class="fas fa-chalkboard-user me-2" style="margin-right: 10px;"></i>จัดการอาจารย์
    </h1>
    <div style="display: flex; gap: 10px;">
        <a href="?action=add" class="btn btn-primary">
            <i class="fas fa-user-plus me-1" style="margin-right: 10px;"></i>เพิ่มอาจารย์ใหม่
        </a>
        <a href="supervision.php" class="btn btn-secondary">
    ...
    <table class="tbl">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    <code
                        style="background: #f0f0f0; padding: 4px 8px; border-radius: 4px;"><?= h($t['username']) ?></code>
                </td>
                <td>
                    <strong><?= h($t['first_name'].' '.$t['last_name']) ?></strong>
                </td>
                <td><?= h($t['email']) ?></td>
                <td><?= h($t['department']) ?></td>
                <td><?= h($t['phone']) ?></td>
                <td style="text-align: right; gap: 8px;">
                    <a href="?action=edit&id=<?= (int)$t['teacher_id'] ?>" class="btn btn-sm btn-warning">
                        <i class="fas fa-edit me-1" style="margin-right: 10px;"></i>แก้ไข
                    </a>
                    <a href="?action=delete&id=<?= (int)$t['teacher_id'] ?>" class="btn btn-sm btn-danger"
...
<?php endif;
// จบการทำงานของหน้าจอแสดงรายการ
      } 

// ==============================================================
// ส่วนที่ 2: หน้าจอสำหรับฟอร์ม "เพิ่ม" หรือ "แก้ไข" ข้อมูลอาจารย์
// ==============================================================
  
elseif ($action === 'add' || $action === 'edit') {
    $teacher = null;

    // ถ้าเป็นการ "แก้ไข" ให้ไปดึงข้อมูลอาจารย์คนนั้นจากฐานข้อมูลมาก่อน
    if ($action === 'edit') {
...
<div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
    <h1>
        <i class="fas fa-chalkboard-user me-2"
            style="margin-right: 10px;"></i><?= $action === 'add' ? 'เพิ่มอาจารย์ใหม่' : 'แก้ไขอาจารย์' ?>
    </h1>
</div>
...
<div class="card card-form">
    ...
    <form method="POST" class="form" style="padding:24px">
        ...
        <div class="row">
            ...
            <label>เบอร์โทร
                <input type="text" name="phone" value="<?= $teacher ? h($teacher['phone']) : '' ?>"
                    placeholder="เช่น 089-xxx-xxxx">
            </label>
        </div>

        <label>สาขา/แผนก
            <input type="text" name="department" value="<?= $teacher ? h($teacher['department']) : '' ?>"
